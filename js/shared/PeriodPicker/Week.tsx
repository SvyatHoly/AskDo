...
function Week({ week, locale, handlePress, is6Weeks, disabledBeforeToday, disabledAfterToday, style }: Props) {
  const renderDayNames = () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const DayComponent = day.date ? (
        <TouchableOpacity
          disabled={(disabledBeforeToday && day.isBeforeToday) || (disabledAfterToday && day.isAfterToday)}
          style={{
            flex: 1,
